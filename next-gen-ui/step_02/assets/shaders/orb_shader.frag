...
// Compute area light illuminance from: Moving Frostbite to Physically Based Rendering 3.0, Siggraph 2014
...
float evalIlluminanceDisk(vec3 N, vec3 L, vec3 lightN, float lightRadius, float lightDistSqr) {
	float cosTheta = dot(N, L);
	float lightRSqr = lightRadius*lightRadius;

	float sinSigmaSqr = lightRSqr/(lightRSqr + max(lightRSqr, lightDistSqr));

	float illuminance = illuminanceSphereOrDisk(cosTheta, sinSigmaSqr) * clamp(dot(lightN, -L), 0.0, 1.0);
...
vec3 brdf_eval(vec3 N, vec3 L, vec3 H, vec3 V, vec3 albedo, float roughness, float metalness, vec3 Li) {
...
vec2 fbm_sphere_sdf(vec3 center, float radius, vec3 pos) {
	...
	return vec2(d + dd - radius, mix(0.4, 1.0, float(d - (radius+1.0*amp) > 0.0)));
}
...
vec3 sample_normal(vec3 pos) {
	...
	for (int i = 0; i < NORMAL_SDF_SAMPLE_COUNT; ++i) {
		result += normalSampleOffsets[i]*sampleScale*sample_scene(pos + normalSampleOffsets[i]).x;
	}
...
void sample_camera_ray(out vec3 origin, out vec3 direction, in mat3 cam, vec3 eye, vec2 uv) {
...
vec4 pixel_color(vec3 o, vec3 d, vec3 lightP, vec3 lightN, float lightR, vec3 lightAtt, vec3 lightLumP) {
	...
	if (t >= 0.0) {
		...
		//Li = sample_disk_light(L, P, N, lightP, lightN, lightR, lightAtt, lightLumP);
		//Lo += S * brdf_eval(N, L, normalize(L + V), V, uAlbedo, uRoughness, uMetalness, Li);

		Li = sample_disk_light(L, P, R, lightP, lightN, lightR, lightAtt, lightLumP);
		Lo += (1.0 - S) * brdf_eval(-N, L, normalize(L + R), R, uAlbedo, uRoughness, uMetalness, Li);
...
void main() {
	...
	vec4 hdrColor = abs(pixel_color(o, d, lightP, lightN, uLightR, lightAtt, uLightLumP));
